...
// h = 2 hasta h = n
     ...
     */
    @Override
    public void armarArbolBinario(int i, int j, TElementoAB[] elementos, TArbolBB elArbolBB ) {
        TElementoAB unNodo;
        int unaraiz;
        if (i < j) {
            unaraiz = R[i][j];
            unNodo = new TElementoAB(elementos[unaraiz].getEtiqueta(), elementos[unaraiz].getDatos());
