...
public class TElementoAB<T> implements IElementoAB<T> {
    ...
    @Override
    public void obtenerDirectorio(TArbolBB indice, Comparable laEspecialidad) {
        Medico medico= (Medico)this.getDatos();
        if (medico.getEspecialidad().equals(laEspecialidad)) {
            indice.insertar(new TElementoAB(medico.getNombre(), medico));
        }
        if (hijoIzq != null) {
            hijoIzq.obtenerDirectorio(indice, laEspecialidad);
        }
        if (hijoDer != null) {
            hijoDer.obtenerDirectorio(indice, laEspecialidad);
        } 
    }
    @Override
...
// tiene los dos hijos, buscamos el lexicograficamente anterior
    ...
    public int obtenerTamanio() {
        Integer temp=0;
        if(hijoIzq!=null){
            temp = temp+hijoIzq.obtenerTamanio();
        }
        temp += 1;
        if (hijoDer!=null){
            temp = temp + hijoDer.obtenerTamanio();
        }
        return temp;
    }
